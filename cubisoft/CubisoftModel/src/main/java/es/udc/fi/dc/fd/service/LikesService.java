...
public class LikesService {
	...
	public static final String SUCESS_LIKED_POST = "Post liked sucessfully.";

	public static final String ALREADY_LIKED_POST_ERROR = "Post already liked.";

	public static final String SUCESS_UNLIKED_POST = "Post unliked sucesfully";

	public static final String POST_NOT_LIKED_YET_ERROR = "Post not liked yet.";

	/*
	 * @Transactional public Likes save(Likes likes) { return
	...
	public String likePost(Long postId, String authorEmail) throws InstanceNotFoundException {
		...
		if (existLikes(author, post)) {
			...
			try {
				...
				/*
				 * } catch (InstanceNotFoundException e) { logger.log(Level.INFO,
				 * e.getMessage(), e); //Unreachable code?
	...
	public String unlikePost(Long postId, String authorEmail) throws InstanceNotFoundException, NotLikedYetException {
		...
		if (!existLikes(author, post)) {
			...
			try {
				 ...
				 * catch (InstanceNotFoundException e) { logger.log(Level.INFO, e.getMessage(),
				 * e); //Unreachable code? }
