...
public class PictureService {
	...
	public Picture setPictureTags(Picture picture, List<String> tags_text) {
		...
		for (String tag_text : tags_text) {
			...
			// System.out.println("Esta wea sigue siendo: " + pictureTag + "
			// porque encontre
			// " + pictureTag.getTag()
	 ...
	 *            userAuthenticated the user that is the author of the
	 *            image @param folderPath the destiny folder path @return the
	 *            multipart file generated by the method @throws
	...
	public String formatDescriptionToHTML(String description) {
		...
		for (String word : description.split("\\s+")) {
			if (word.startsWith("#")) {
				result += "<a href=\"../post/globalFeed?hashtags=" + word.substring(1, word.length())
