...
public class PictureService {
	 ...
	 *
	 * @param picture   the picture
	 * @param tags_text the text of the tags that we want to add to the image.
	 ...
	 *
	 * @param uploadPictureForm the upload picture form that contains the form
	 *                          data. @param userAuthenticated the user that is the
	 *                          author of the image @param folderPath the destiny
	 *                          folder path @return the multipart file generated by
	 *                          the method @throws
	...
	public MultipartFile uploadPicture(UploadPictureForm uploadPictureForm, Principal userAuthenticated,
			String folderPath) throws InstanceNotFoundException {
		...
		if (!file.isEmpty()) {
			try {
				...
				if (uploadPictureForm.getExpiration() != 0) {
					// TODO comprobar que le entra un número y no otra cosa
					storyService.newStory(p, author, uploadPictureForm.getExpiration());
				} else {
	 ...
	 *
	 * @param hashtags the hashtags text
